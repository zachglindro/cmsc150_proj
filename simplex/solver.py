...
# n column is the original objective function
# form: ax=b
...
price_index = col_list.index('Price/Serving')
last_index = len(col_list) - 1
col_list[price_index], col_list[last_index] = col_list[last_index], col_list[price_index]
...
# Each food has max 10 serving
# Insert new diagonal matrix before last column
for i in range(len(test_data.index)-1):
    test_data.insert(len(test_data.columns)-1, f'max_{i}', 0)
